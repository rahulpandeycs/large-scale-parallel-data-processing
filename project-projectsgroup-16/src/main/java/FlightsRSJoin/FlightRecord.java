...
public class FlightRecord implements WritableComparable<FlightRecord> {
    ...
    private Text originCityName;

    private Text destinationAirportId;
    private Text destinationCityName;
    ...
    public FlightRecord(FlightRecord flightRecord) {
        this(
    ...
    public FlightRecord(String flDate, String originAirportId, String originCityName, String destinationAirportId,
    ...
    public FlightRecord(Text flDate, Text originAirportId, Text originCityName, Text destinationAirportId,
    ...
    FlightRecord() {
        ...
        destinationAirportId = new Text();
        destinationCityName = new Text();
    ...
    public int compareRecordsOnDate(FlightRecord flightRecord) {
        ...
        Double localArrTime = Double.parseDouble(arrTime.toString()) + 100;
        Double localDepTime = Double.parseDouble(flightRecord.depTime.toString());

        if (inputDate.compareTo(currentObjectDate) == 0) {
            return localDepTime.compareTo(localArrTime);
        } else {
            if (inputDate.getYear() - currentObjectDate.getYear() == 0) {
//                System.out.println("Same year");
                if (inputDate.getMonthValue() - currentObjectDate.getMonthValue() == 0) {
//                    System.out.println("Same month");
                    if (inputDate.getDayOfMonth() - currentObjectDate.getDayOfMonth() < 7) {
//                        System.out.println(currentObjectDate.getDayOfMonth());
...
//                        System.out.println("date out of range");
                ...
                } else if ((inputDate.getMonthValue() + 1) == currentObjectDate.getMonthValue()) {
//                    System.out.println("they are within one month");
                    if ((inputDate.getMonth().maxLength() - inputDate.getDayOfMonth() + currentObjectDate.getDayOfMonth()) < 7) {
//                        System.out.println("date within range, month diff");
                        return 1;
                    }
                    return -1;
                }
//                System.out.println("More than 1 month");
                return -1;
            }
            return -1;
        }
    ...
    @Override
    public int compareTo(FlightRecord flightRecord) { //Compare based on dates
        LocalDate inputDate = LocalDate.parse(getFormattedDate(flightRecord.flDate.toString()));
        LocalDate currentObjectDate = LocalDate.parse(getFormattedDate(this.flDate.toString()));
    ...
    public static FlightRecord getFlightRecord(String value) {
    ...
    public static void main(String[] args) {
        //mmddyy
        var fl = FlightRecord.getFlightRecord("2/1/14,13930,Chicago, IL,12953,New York, NY,1436,1726,0,733,0,15,0,0,21");
        var fl2 = FlightRecord.getFlightRecord("1/29/14,13930,Chicago, IL,12953,New York, NY,1436,1726,0,733,0,15,0,0,21");
        assert fl != null;
        assert fl2 != null;
        System.out.println(fl.compareRecordsOnDate(fl2));
    }
}
